...
function make_setpagevalues(){
    ...
    function set_page_values(target_props,value,options){
        
        let target   = document;
        let selector = ":scope";
        let setelements = [];
        
        if( is_object(target_props) ){
            selector = target_props["selector"]||selector;
            target   = target_props["target"]  ||target;
            setelements = target.querySelectorAll(selector);
        }else if( is_string(target_props) ){
            selector = target_props;
            setelements = target.querySelectorAll(selector);
        }else if( is_typekey(target_props,"HTML") ){
            target   = target_props;
            // setelements = [target_props];
            setelements = target.querySelectorAll(selector);
        }
...
function make_getpagevalues(){
    ...
    function get_page_values(target_props,argdict){
        ...
        if( is_object(target_props) ){
            selector = target_props["selector"]||selector;
            target   = target_props["target"]  ||target;
            getelements = target.querySelectorAll(selector);
        }else if( is_string(target_props) ){
            selector = target_props;
            getelements = target.querySelectorAll(selector);
        }else if( is_typekey(target_props,"HTML") ){
            target   = target_props;
            // getelements = [target_props];
            getelements = target.querySelectorAll(selector);
        }
...
function imitatelist_correct(target){
    ...
    for( let i = -1; ++i < target.children.length; ){
        ...
        result.push(get_elm_value(child));
    }
    return result;
}
//////////////// page data controller ////////////////
...
function check_undercount(elm,classname){
    const ret = elm.classList.contains(classname);
    let max_c = 0;
    const t = elm.querySelectorAll(":scope > *");
    for( let i = -1; ++i < t.length; ){
        const cc = check_undercount(t[i],classname);
        max_c = Math.max(cc,max_c);
    }
    return ret+max_c;
}

//////////////// accordion controls ////////////////
function control_max_height(accordion,panel,first=false){
    let controlHeight = panel.scrollHeight;
    if( !panel.style.maxHeight ) {
        // resize ovserveの為にmh=sh+1
    ...
    control_max_height_closest(panel,controlHeight,first);
}

function control_max_height_closest(panel,controlHeight,first=false){
    // 親を遡ってパネルの高さを調整
    ...
    while( target ){
        ...
        if( baseheight === 0 || !target.classList.contains("active") ){
            break;
        }
        // resize ovserveの為にmh=sh+1
        target.style.maxHeight = (baseheight + controlHeight + 1)+"px";
...
function get_panel(accordion){
    let   panel = null;
    const type = accordion.type;
    if( type === "radio" ){
        if( accordion.getAttribute("data-rawname") ){
            const key = accordion.getAttribute("data-rawname");
            panel = document.getElementById(key+"_panel");
        }
    }
    if( accordion.id ){
        panel = document.getElementById(accordion.id+"_panel");
    }
    if( !panel ){
...
function toggle_accordion(first=false) {
    ...
    if( !first ){
        // pass
    }else if( this.classList.contains("active") && panel.style.maxHeight ){
        return;
    }else if( !this.classList.contains("active") && !panel.style.maxHeight ){
        return;
    }
    
    control_max_height(this,panel,first);
...
function toggle_accordion_radio(first=false) {
    ...
    if( this.checked && panel.style.maxHeight ){
        return;
    }else if( !this.checked && !panel.style.maxHeight ){
        return;
    }
    ...
    if( this.classList.contains("active") ){
        ...
        const targets = document.querySelectorAll("input[type='radio'][name='"+name+"'].active:not(:checked)");
        for( let i = -1; ++i < targets.length; ){
...
function control_accordions(top_node=document,target_only=false){
    ...
    // control accordions func
    {
        const under_tree = top_node.querySelectorAll(".accordion:not(.called)");
        const accordions = target_only?[top_node]:under_tree;
        for( let i = 0; i < accordions.length; i++ ){
            const target = accordions[i];
            
            if( target.classList.contains("called") ){ continue; }
            
            const type   = types[target.type];
            const func   = funcs[type];
            target.addEventListener("click",func.bind(target,false));
            target.classList.add("called");
            
            const panel  = get_panel(target);
            observe_scrollheight(panel);
        }
    }
    
    // control default open
    {
        const key = ".accordion.open:not(.active,:checked)";
        const under_tree = top_node.querySelectorAll(key);
        const accordions = target_only?[top_node]:under_tree;
        for( let i = 0; i < accordions.length; i++ ){
            const target = accordions[i];
            if( !target.classList.contains("open") ){ continue; }
...
function observe_scrollheight(target){
    // max height と scroll height が同一の場合、不発する
    // max heightを常にshの+1で設定してトリガーを有効化する
    const observer = new ResizeObserver(resize_panel);
    observer.observe(target);
}

function resize_panel(entry,observer){
    update_max_height(entry[0].target);
}

function update_max_height(target){
    if( !target.style.maxHeight ){ return }
    const mh_str = target.style.maxHeight;
    const mh = mh_str.slice(0,mh_str.length-2)*1;
    const sh = target.scrollHeight;
    if( mh !== sh+1 ){
        // resize ovserveの為にmh=sh+1
        target.style.maxHeight = (sh+1)+"px";
        control_max_height_closest(target,sh-mh,false);
    }
}
...
function make_inputstyle_dict(input_length){
    const px = getTextWidth("m",getCanvasFontSize())*input_length;
    return {margin:0,width:px?px:"auto",block:true,text_align:undefined};
}
function make_inputstyle(input_length){
    const dict = make_inputstyle_dict(input_length)
    return make_styletext(dict);
}
// num input style {margin:0,width:input.length*4,block:true,text_align:"right"}
function make_numstyle_dict(input_length){
    const px = getTextWidth("0",getCanvasFontSize())*input_length;
    return {margin:0,width:px?px:"auto",block:true,text_align:"right"};
}
function make_numstyle(input_length){
    const dict = make_numstyle_dict(input_length);
    return make_styletext(dict);
}
...
export {
    ...
    div_correct,
    imitatetable_expand,
    imitatetable_correct,
    imitatelist_expand,
    imitatelist_correct,
    default_pattern,
    control_max_height,
    toggle_accordion,
    toggle_accordion_radio,
    control_accordions,
    observe_scrollheight,
    update_max_height,
    drag,
    make_drop,
    prevent_default,
